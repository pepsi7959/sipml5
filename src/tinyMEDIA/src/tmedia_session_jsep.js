...
tmedia_session_jsep01.prototype.__get_lo = function () {
    ...
    if (!this.o_pc && !this.b_lo_held) {
        ...
        this.o_pc = new window.RTCPeerConnection(
                (o_iceServers && !o_iceServers.length) ? null : { iceServers: o_iceServers,
                    iceTransportPolicy: "relay",
                     rtcpMuxPolicy: "negotiate" }, // empty array is used to disable STUN/TURN.
                this.o_media_constraints
        );
        if( tmedia_session_jsep01.mozThis ){
            tsk_utils_log_info("tmedia_session_jsep01.mozThis is exist!!!" );
        }else{
            tsk_utils_log_info("tmedia_session_jsep01.mozThis is null" );
        }
    ...
    if (!this.o_sdp_lo && !this.b_sdp_lo_pending) {
        ...
        tsk_utils_log_info(" !this.o_sdp_lo && !this.b_sdp_lo_pending " );
